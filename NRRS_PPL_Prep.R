...
for(i in 1:length(bin_list)){
  ...
  for(j in year_list){
    ...
    names(NRRS@data)[names(NRRS@data) == "NRRS_dat_select[, ncol(NRRS_dat_select)]"] <- paste0("PPL_Frequency_",i,"_",j)
  }
  
  NRRS@data <- cbind(NRRS@data,as.data.frame(rowSums(NRRS@data[,11:17])))
  names(NRRS@data)[names(NRRS@data) == "rowSums(NRRS@data[, 11:17])"] <- paste0("PPL_Frequency_",i,"_sum")
